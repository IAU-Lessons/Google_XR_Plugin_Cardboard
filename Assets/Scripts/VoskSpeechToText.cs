...
public class VoskSpeechToText : MonoBehaviour
{
	...
	public void ResetRecognizer()
	{
		if (_recognizer != null)
		{
			...
			Debug.Log("Vosk recognizer ve buffer'lar resetlendi");
		}
	}
	...
	private async Task ThreadedWork()
	{
		...
		if (!_recognizerReady)
		{
			...
			// Model ve gramer yükleme işlemleri
		...
		while (_running)
		{
			...
			{
				// EĞER CÜMLE BİTTİYSE (Sessizlik algılandıysa)
				if (_recognizer.AcceptWaveform(voiceResult, voiceResult.Length))
				{
					var result = _recognizer.Result();
					_threadedResultQueue.Enqueue(result); // Final sonucu gönder
				}
				// EĞER HALA KONUŞULUYORSA (Anlık akış)
				else 
				{
					var partialResult = _recognizer.PartialResult();
					_threadedResultQueue.Enqueue(partialResult); // Anlık, yarım sonucu gönder
				}
			}
			else
			{
				// Veri gelmesini bekle
				await Task.Delay(10); // 100ms çok uzun olabilir, 10ms daha akıcı yapar
			}
		}
